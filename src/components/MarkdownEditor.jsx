...
function MarkdownEditor({ markdown, setMarkdown, isDark, settings }) {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const totalVisibleLines = Math.ceil(
        (visibleHeight + scrollTop) / lineHeight
      );
    ...
    >
      ...
      <div className="relative">
        {settings.showLineNumbers && (
          <div
            ref={lineNumbersRef}
            className={`absolute left-0 top-2 sm:top-4 bottom-2 sm:bottom-4 px-1 sm:px-2 text-right select-none overflow-hidden ${
              isDark
        ...
        <textarea
          ...
          className={`w-full h-[300px] sm:h-[500px] rounded-lg focus:ring-2 focus:ring-purple-500 focus:outline-none ${
